...
if (isset($_GET['fetch_requests']) && $_GET['fetch_requests'] == '1') {
    ...
    if ($requests && $requests->num_rows > 0) {
        while($row = $requests->fetch_assoc()) {
            ...
            echo '<button class="btn btn-sm btn-outline-primary rounded-pill px-3" onclick="openViewRequestModal(\'' . htmlspecialchars($row['request_group_id'], ENT_QUOTES) . '\', \'office\')">';
        ...
        echo '<tr><td colspan="6" class="text-center text-muted py-4">Walang nakabinbing Office Supply Requests.</td></tr>';
    }
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_request'])) {
    ...
    if ($action === 'Approved') {
        ...
        foreach ($quantities as $req_id => $new_qty) {
            ...
            $stmt_r = $conn->prepare("SELECT item_id FROM supply_requests WHERE id = ? AND request_group_id = ?");
            ...
            if ($req_res) {
                ...
                $stmt_chk = $conn->prepare("SELECT actual_stocks, item_name FROM items WHERE id = ?");
        ...
        if ($can_approve && !empty($items_to_process)) {
            foreach ($items_to_process as $item) {
                $stmt_up_req = $conn->prepare("UPDATE supply_requests SET quantity = ?, status = 'Approved', approved_at = NOW() WHERE id = ?");
                $stmt_up_req->bind_param("ii", $item['qty'], $item['req_id']);
                $stmt_up_req->execute();

                $stmt_deduct = $conn->prepare("UPDATE items SET actual_stocks = actual_stocks - ? WHERE id = ?");
                $stmt_deduct->bind_param("ii", $item['qty'], $item['item_id']);
                $stmt_deduct->execute();
            }

            $stmt_rej_rem = $conn->prepare("UPDATE supply_requests SET status = 'Rejected' WHERE request_group_id = ? AND status = 'Pending'");
            $stmt_rej_rem->bind_param("s", $group_id);
            $stmt_rej_rem->execute();

            // Notify user
            $stmt_usr = $conn->prepare("SELECT user_id FROM supply_requests WHERE request_group_id = ? LIMIT 1");
        ...
        $stmt_rej = $conn->prepare("UPDATE supply_requests SET status = 'Rejected' WHERE request_group_id = ? AND status = 'Pending'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_office_item'])) {
    ...
    $stmt_log = $conn->prepare("INSERT INTO stock_history (item_id, item_name, category, previous_stock, new_stock, added_qty, updated_by) VALUES (?, ?, 'Office', 0, ?, ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_stock'])) {
    ...
    $stmt_old = $conn->prepare("SELECT item_name, actual_stocks, image FROM items WHERE id = ?");
    ...
    if ($new_image) {
        ...
        $stmt_up = $conn->prepare("UPDATE items SET actual_stocks = ?, image = ? WHERE id = ?");
        $stmt_up->bind_param("isi", $new_stock, $new_image, $item_id);
    } else {
        $stmt_up = $conn->prepare("UPDATE items SET actual_stocks = ? WHERE id = ?");
    ...
    // Log to stock history
    $updater = $_SESSION['username'] ?? 'Admin';
    $stmt_log = $conn->prepare("INSERT INTO stock_history (item_id, item_name, category, previous_stock, new_stock, added_qty, updated_by) VALUES (?, ?, 'Office', ?, ?, ?, ?)");
    $stmt_log->bind_param("isiiis", $item_id, $item_name, $prev_stock, $new_stock, $added_qty, $updater);
    $stmt_log->execute();

    sendResponse("Matagumpay na na-update ang office supply inventory!", true);
}
...
$out_of_stock_count = $conn->query("SELECT COUNT(*) as cnt FROM items WHERE actual_stocks <= 0")->fetch_assoc()['cnt'] ?? 0;
?>
...
<head>
    ...
    <title>Office Supplies Admin - SIBTECH</title>
...
<nav class="navbar navbar-expand-lg navbar-dark navbar-admin py-3 shadow-sm">
    <div class="container">
        <a class="navbar-brand fw-bold d-flex align-items-center text-white" href="admin_dashboard.php">
            ...
            <span>SIBTECH <span class="fw-light opacity-75">Office Supplies Admin</span></span>
        </a>
...
<div class="container py-4">
    ...
    <div class="card p-4 mb-4">
        <h5 class="fw-bold text-dark mb-3"><i class="fa-solid fa-clipboard-list text-logo-blue me-2"></i>Pending Office Supply Requests</h5>
    ...
    <div class="card p-4">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
            <h5 class="fw-bold text-dark mb-0"><i class="fa-solid fa-boxes-stacked text-logo-blue me-2"></i>Office Supply Inventory</h5>
            <div class="d-flex gap-2">
                <a href="export_out_of_stock.php?type=office" class="btn btn-success btn-sm rounded-pill px-3 fw-bold">
        ...
        <div class="table-responsive table-container">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    <?php if($office_inventory && $office_inventory->num_rows > 0): ?>
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="7" class="text-center text-muted py-4">Walang items sa office inventory.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
...
function openViewRequestModal(groupId) {
    ...
    $.ajax({
        url: 'admin_dashboard.php?fetch_request_details=1&group_id=' + encodeURIComponent(groupId) + '&type=office',
        type: 'GET',
